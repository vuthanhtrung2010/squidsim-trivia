...
module.exports = async (client, message) => {
  try {
    ...
    const missingPermissions = requiredPermissions.filter(perm => !message.guild.me.permissions.has(perm));
    if (missingPermissions.length > 0) {
      const missing = missingPermissions.map(perm => `**${perm}**`).join(", ");
      return message.reply(`:x: **I am missing the following permissions: ${missing}**`).catch(console.error);
    }
        ...
        return message.reply({
          embeds: [
            new Discord.MessageEmbed()
              .setColor(es.color)
              .setTitle(`<a:yes:958653519513133078> **To see all Commands type: \`${prefix}help\`!**`),
          ],
        }).catch(console.error);
      return;
    }
          ...
          embeds: [
            new Discord.MessageEmbed()
              ...
              .setAuthor({
                ...
                iconURL: message.member.user.displayAvatarURL({ dynamic: true, size: 64 }) || message.member.user.defaultAvatarURL,
                url: "https://links.trung.is-a.dev",
              })
              .setDescription(`<:no:948483017993769041> Please wait ${formattedTimeLeft} more second(s) before reusing the \`${command.name}\` command.`),
          ],
        }).catch(console.error);
      }
    }
